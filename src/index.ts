...
export const convertWordFiles = async (pathFile: string, extOutput: string, nameOutput: string outputDir: string): Promise<string> => {
  ...
  return path.join(outputDir, `${nameOutput}.${extOutput}`);
}
